  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={(v) => !v && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl z-[120] w-[calc(100%-2rem)] max-w-3xl max-h-[90vh] overflow-y-auto p-0">
          <BuildTaskForm
            title={title}
            dueDate={dueDate}
            dueTime={dueTime}
